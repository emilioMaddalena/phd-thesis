...
alpha = 0.1;
lengthscale = 1;
kernel = @(x1,x2) (1 + dist(x1,x2').^2 / (2*alpha*lengthscale^2)).^(-alpha);
...
% custom formatting
title('rational quadratic','Interpreter','latex')

yl = ylabel('$k(x,0)$','Interpreter','latex');
ylim([0.7 1.05]); yticks([])
ref = ylim;
...
exportgraphics(gcf, '../images/chap2_kernel_rq.pdf','ContentType', 'vector');
