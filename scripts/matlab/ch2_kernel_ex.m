...
alpha = 0.1;
lengthscale = 1;
kernel = @(x1,x2) exp(-dist(x1,x2') / (2*lengthscale^2));
...
% custom formatting
title('exponential','Interpreter','latex')

yl = ylabel('$k(x,0)$','Interpreter','latex');
ylim([0.6 1.02]); yticks([])
ref = ylim;
...
exportgraphics(gcf, '../images/chap2_kernel_ex.pdf','ContentType', 'vector');
